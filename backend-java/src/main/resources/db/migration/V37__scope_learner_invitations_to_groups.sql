-- Invitations become group-scoped: the group leader (not the enterprise owner)
...
ALTER TABLE public.learner_invitations
    ADD COLUMN IF NOT EXISTS enterprise_group_id BIGINT NULL
        REFERENCES public.enterprise_groups(enterprise_group_id),
    ADD COLUMN IF NOT EXISTS invited_by BIGINT NULL
        REFERENCES public.users(user_id);

CREATE INDEX IF NOT EXISTS idx_learner_invitations_group
    ON public.learner_invitations(enterprise_group_id);
